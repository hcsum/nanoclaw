...
async function runScript(script, args) {
    const scriptPath = path.join(process.cwd(), '.claude', 'skills', 'web-cafe', 'scripts', `${script}.ts`);
 ...
 */
export async function handleWebCafeIpc(data, sourceGroup, isMain, dataDir) {
    ...
    if (result.success) {
        ...
        logger.warn({ type, sourceGroup, requestId, message: result.message }, 'Web.Cafe IPC task failed');
    }
    return true;
}
