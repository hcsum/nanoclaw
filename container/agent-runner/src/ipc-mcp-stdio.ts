...
async function waitForWebCafeResult(
  ...
  const resultFile = path.join(WEB_CAFE_RESULTS_DIR, `${requestId}.json`);
...
server.tool(
  'web_cafe_search',
  'Search Web.Cafe (哥飞的朋友们) for posts, tutorials, and experiences about SEO, keywords, and overseas entrepreneurship. Main group only.',
  {
    query: z.string().describe('Search query (e.g. "keyword research", "SEO tools")'),
    limit: z
      .number()
      .int()
      .min(1)
  ...
  async (args) => {
    if (!isMain) {
      return {
        content: [{ type: 'text' as const, text: 'Only the main group can use Web.Cafe tools.' }],
        isError: true,
      };
    }
...
server.tool(
  'web_cafe_read',
  'Read full content from a specific Web.Cafe URL (post, tutorial, or experience). Main group only.',
  {
...
server.tool(
  'web_cafe_browse',
  'Browse recent posts from a specific Web.Cafe section. Main group only.',
  {
    section: z
      .enum(['topics', 'tutorials', 'experiences', 'all'])
      .default('all')
      .describe('Section to browse: topics, tutorials, experiences, or all'),
    limit: z
      .number()
      .int()
      .min(1)
...
async function waitForBrowserResult(
  ...
  while (elapsed < timeoutMs) {
    if (fs.existsSync(resultFile)) {
      try {
        ...
        return { success: false, message: `Failed to parse result: ${err}` };
      }
    }
  ...
  return { success: false, message: 'Browser task timed out' };
}

server.tool(
  'browser_open',
  'Navigate to a URL in the browser. Opens a visible Chrome window on the host machine.',
  {
    url: z.string().describe('The URL to navigate to'),
  },
  async (args) => {
    const requestId = `browser-open-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    writeIpcFile(TASKS_DIR, {
      type: 'browser_open',
      taskId: requestId,
      input: { url: args.url },
...
server.tool(
  ...
  {
    interactive: z.boolean().default(true).describe('Only show interactive elements (default: true)'),
    limit: z.number().int().min(1).max(100).default(50).describe('Max elements to return (default: 50)'),
  },
  async (args) => {
    const requestId = `browser-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    writeIpcFile(TASKS_DIR, {
      type: 'browser_snapshot',
      taskId: requestId,
      input: { interactive: args.interactive, limit: args.limit },
      groupFolder,
      timestamp: new Date().toISOString(),
    });
...
server.tool(
  'browser_action',
  'Perform an action on the page (click, fill, scroll, press, wait). Use element references from browser_snapshot.',
  {
    action: z.enum(['click', 'fill', 'scroll', 'press', 'wait']).describe('Action type'),
    selector: z.string().optional().describe('CSS selector or @ref from snapshot (for click/fill)'),
    value: z.string().optional().describe('Value for fill or press actions'),
    direction: z.enum(['up', 'down']).default('down').describe('Scroll direction (default: down)'),
    amount: z.number().optional().describe('Scroll amount in pixels or wait time in ms'),
  },
  async (args) => {
    const requestId = `browser-action-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    writeIpcFile(TASKS_DIR, {
      type: 'browser_action',
      taskId: requestId,
      input: {
...
server.tool(
  'browser_read',
  'Extract text content from the current page. Removes scripts, styles, and navigation elements.',
  {
    selector: z.string().optional().describe('Optional CSS selector to scope reading to a specific element'),
  },
  async (args) => {
    const requestId = `browser-read-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    writeIpcFile(TASKS_DIR, {
      type: 'browser_read',
      taskId: requestId,
